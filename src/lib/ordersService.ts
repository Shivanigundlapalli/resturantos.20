...
export class OrdersService {
  ...
  async updateCategory(id: string, cat: any) {
    return this.repository.updateCategory(id, cat);
  }
  ...
  async updateMenuItem(id: string, item: any) {
    return this.repository.updateMenuItem(id, item);
  }
  ...
  async placeOrder(orderPayload: {
    ...
    for (const item of orderPayload.items) {
      if (item.quantity <= 0) {
        throw new Error(`Invalid quantity ${item.quantity} for item ${item.menuItemId}`);
      }
      ...
      if (menuObj.status === "Sold Out") {
        throw new Error(`Sorry, ${menuObj.name} is currently Sold Out.`);
      }
    ...
    return this.repository.createOrder({
      ...
      estimated_prep_time: orderPayload.estimated_prep_time
    });
  }
