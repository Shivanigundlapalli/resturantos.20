...
export async function generateAndSendBusinessReport(dbState?: any) {
  ...
  try {
    ...
    let lowStockNames = '• No Data Available';
    const dbInstance = getPool();
    if (dbInstance) {
      try {
        const lowStockNamesQuery = await dbInstance.query(`
        ...
        `);
        lowStockNames = lowStockNamesQuery.rows.length > 0 
          ? lowStockNamesQuery.rows.map(r => `• ${r.name}`).join('\n') 
          : '• All stock healthy';
      } catch (err) {
        console.error("Failed to fetch low stock names:", err);
      }
    } else if (dbState && dbState.inventory) {
      const lowItems = dbState.inventory.filter((i: any) => i.currentQty <= i.reorderLevel && i.currentQty > 0).slice(0, 5);
      lowStockNames = lowItems.length > 0 
        ? lowItems.map((i: any) => `• ${i.name}`).join('\n') 
        : '• All stock healthy';
    }
    ...
    try {
      ...
      if (db) {
        ...
        console.log("[8] Database offline, skipping report history log");
      }
    } catch (err) {
