...
export default function InventoryView({ 
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-zinc-950 px-4 py-6 sm:px-8 overflow-y-auto font-sans select-none animate-fade-in text-zinc-100 relative">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">Real-time stock tracking and supplier management</p>
        </div>
        <button 
          onClick={openAddModal}
          className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2.5 px-5 rounded-xl shadow-lg shadow-emerald-900/20 transition-all flex items-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        <div className="bg-zinc-900/50 backdrop-blur-md border border-zinc-800/80 p-5 rounded-2xl flex flex-col gap-2">
          <div className="flex items-center gap-2 text-zinc-400">
            <Package className="w-4 h-4" />
            <span className="text-xs font-bold uppercase tracking-wider">Total Ingredients</span>
          </div>
          <span className="text-3xl font-black text-zinc-100">{totalIngredients}</span>
        </div>
        ...
        <div className="bg-amber-900/10 backdrop-blur-md border border-amber-500/20 p-5 rounded-2xl flex flex-col gap-2">
          <div className="flex items-center gap-2 text-amber-500">
            <AlertTriangle className="w-4 h-4" />
            <span className="text-xs font-bold uppercase tracking-wider">Low Stock</span>
          </div>
          <span className="text-3xl font-black text-amber-500">{lowStockCount}</span>
        </div>
        ...
        <div className="bg-zinc-900/50 backdrop-blur-md border border-zinc-800/80 p-5 rounded-2xl flex flex-col gap-2">
          ...
          <span className="text-3xl font-black text-amber-500">₹{totalValue.toLocaleString()}</span>
        </div>
      </div>

      {/* Toolbar */}
      <div className="bg-zinc-900/50 backdrop-blur-md border border-zinc-800/80 p-4 rounded-2xl flex flex-col lg:flex-row justify-between items-center gap-4 mb-6">
        <div className="relative w-full lg:w-96">
          ...
          <input
            ...
            className="w-full bg-zinc-950 border border-zinc-800 pl-10 pr-4 py-2 rounded-xl text-sm font-medium text-zinc-100 focus:outline-none focus:border-amber-500 transition-colors"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          <select 
            ...
            className="bg-zinc-950 border border-zinc-800 text-zinc-300 text-xs font-bold rounded-xl px-3 py-2.5 focus:outline-none focus:border-amber-500"
          >
          ...
          <button onClick={fetchInventory} className="p-2.5 bg-zinc-800 hover:bg-zinc-700 rounded-xl transition-colors text-zinc-300">
      ...
      {/* Table */}
      <div className="bg-zinc-900/50 backdrop-blur-md border border-zinc-800/80 rounded-2xl overflow-hidden flex-1 flex flex-col">
        <div className="overflow-x-auto flex-1">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-zinc-950/50 text-zinc-400 font-semibold text-xs uppercase tracking-wider sticky top-0 z-10">
              <tr>
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-zinc-800/30 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <span className="px-2.5 py-1 bg-zinc-800 text-zinc-300 rounded-lg text-xs font-medium">
                          {item.category}
                        </span>
                      </td>
                      <td className="px-6 py-4 font-mono font-bold">
                        {item.currentQty} <span className="text-zinc-500 text-xs">{item.unit}</span>
                      </td>
                      <td className="px-6 py-4 font-mono text-zinc-400">{item.reorderLevel}</td>
                      <td className="px-6 py-4">
                        {isOut ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded-lg text-xs font-bold">
                      ...
                      <td className="px-6 py-4 text-zinc-300">{item.supplierName || "Unknown"}</td>
                      <td className="px-6 py-4 font-mono">₹{item.unitPrice}</td>
                      <td className="px-6 py-4 font-mono font-bold text-amber-500">₹{value.toLocaleString()}</td>
                      ...
                      <td className="px-6 py-4">
                        {item.whatsapp_status === 'Sent' ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-amber-500/10 text-amber-400 border border-amber-500/20 rounded-lg text-[10px] font-bold w-fit">
                          ...
                          <div className="relative group inline-block">
                            ...
                            <div className="absolute right-0 bottom-full mb-2 hidden group-hover:block w-48 bg-zinc-900 border border-zinc-700 text-zinc-300 text-[10px] p-2 rounded shadow-xl z-50 whitespace-normal">
                              <div className="font-bold mb-1 text-red-400">Twilio Error</div>
                              {item.whatsapp_error || "Unknown Error"}
                            </div>
                          </div>
                        ) : (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-zinc-800/50 text-zinc-500 border border-zinc-700/50 rounded-lg text-[10px] font-bold w-fit">
                            ⚪ Never Sent
                          </span>
                        )}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2.5 py-1 rounded-lg text-xs font-medium ${(item.last_notification_type) ? ((item.whatsapp_status === 'Failed' || item.voice_status === 'Failed') ? 'bg-red-500/10 text-red-500' : 'bg-emerald-500/10 text-emerald-400') : 'bg-zinc-800 text-zinc-400'}`}>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => openAdjustModal(item)} className="p-1.5 text-zinc-400 hover:text-amber-400 bg-zinc-800 hover:bg-zinc-700 rounded-md transition-colors" title="Adjust Stock">
                            <RefreshCw className="w-4 h-4" />
                          </button>
                          <button onClick={() => openHistory(item)} className="p-1.5 text-zinc-400 hover:text-blue-400 bg-zinc-800 hover:bg-zinc-700 rounded-md transition-colors" title="History">
                            <History className="w-4 h-4" />
                          </button>
                          <button onClick={() => openEditModal(item)} className="p-1.5 text-zinc-400 hover:text-zinc-100 bg-zinc-800 hover:bg-zinc-700 rounded-md transition-colors" title="Edit">
                            <Edit2 className="w-4 h-4" />
                          </button>
                          <button onClick={() => handleDelete(item.id)} className="p-1.5 text-zinc-400 hover:text-red-400 bg-zinc-800 hover:bg-red-900/30 rounded-md transition-colors" title="Delete">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden flex flex-col">
            <div className="flex justify-between items-center p-5 border-b border-zinc-800">
              ...
              <button onClick={() => setIsAddEditModalOpen(false)} className="text-zinc-400 hover:text-white transition-colors"><X className="w-5 h-5"/></button>
            </div>
            <div className="p-5 space-y-4 flex-1 overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-xs font-bold text-zinc-400 mb-1.5">Ingredient Name</label>
                  <input type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none transition-colors" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-zinc-400 mb-1.5">Category</label>
                  <select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none transition-colors">
                    {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold text-zinc-400 mb-1.5">Supplier</label>
                  <select value={formData.supplierId} onChange={e => setFormData({...formData, supplierId: e.target.value})} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none transition-colors">
                    <option value="">Select Supplier</option>
                    {suppliers.map(s => <option key={s.id} value={s.id}>{s.companyName}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold text-zinc-400 mb-1.5">Current Quantity</label>
                  <input type="number" step="0.01" value={formData.currentQty} onChange={e => setFormData({...formData, currentQty: e.target.value === '' ? '' : parseFloat(e.target.value) || 0})} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none transition-colors" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-zinc-400 mb-1.5">Unit</label>
                  <select value={formData.unit} onChange={e => setFormData({...formData, unit: e.target.value})} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none transition-colors">
                    {UNITS.map(u => <option key={u} value={u}>{u}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold text-zinc-400 mb-1.5">Cost Per Unit (₹)</label>
                  <input type="number" step="0.01" value={formData.unitPrice} onChange={e => setFormData({...formData, unitPrice: e.target.value === '' ? '' : parseFloat(e.target.value) || 0})} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none transition-colors" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-zinc-400 mb-1.5">Reorder Level</label>
                  <input type="number" step="0.01" value={formData.reorderLevel} onChange={e => setFormData({...formData, reorderLevel: e.target.value === '' ? '' : parseFloat(e.target.value) || 0})} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none transition-colors" />
                </div>
              </div>
            </div>
      ...
      {isAdjustModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden flex flex-col">
            <div className="flex justify-between items-center p-5 border-b border-zinc-800">
              <h2 className="text-lg font-black tracking-tight">Adjust Stock</h2>
              <button onClick={() => setIsAdjustModalOpen(false)} className="text-zinc-400 hover:text-white transition-colors"><X className="w-5 h-5"/></button>
            </div>
            <div className="p-5 space-y-5">
              <div className="flex gap-2 p-1 bg-zinc-950 rounded-xl border border-zinc-800">
                <button 
                  ...
                  className={`flex-1 py-2 text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${adjustForm.type === "IN" ? "bg-emerald-500/20 text-emerald-400" : "text-zinc-500 hover:text-zinc-300"}`}
                >
                  <ArrowUpCircle className="w-4 h-4"/> Stock In
                </button>
                <button 
                  onClick={() => setAdjustForm({...adjustForm, type: "OUT", reason: "Cooking"})}
                  className={`flex-1 py-2 text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${adjustForm.type === "OUT" ? "bg-red-500/20 text-red-400" : "text-zinc-500 hover:text-zinc-300"}`}
                >
              ...
              <div>
                <label className="block text-xs font-bold text-zinc-400 mb-1.5">Quantity ({adjustingItem?.unit})</label>
                <input type="number" step="0.01" value={adjustForm.quantity} onChange={e => setAdjustForm({...adjustForm, quantity: e.target.value === '' ? '' : parseFloat(e.target.value) || 0})} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm focus:border-amber-500 focus:outline-none transition-colors" />
              </div>

              <div>
                <label className="block text-xs font-bold text-zinc-400 mb-1.5">Reason</label>
                <select value={adjustForm.reason} onChange={e => setAdjustForm({...adjustForm, reason: e.target.value})} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm focus:border-amber-500 focus:outline-none transition-colors">
                  {adjustForm.type === "IN" ? (
                    <>
            ...
            <div className="p-5 border-t border-zinc-800 flex justify-end gap-3 bg-zinc-950/50">
              <button onClick={() => setIsAdjustModalOpen(false)} className="px-5 py-2.5 rounded-xl font-bold text-sm text-zinc-300 hover:text-white hover:bg-zinc-800 transition-colors">Cancel</button>
              <button onClick={handleAdjustStock} className="px-5 py-2.5 rounded-xl font-bold text-sm bg-amber-500 hover:bg-amber-400 text-zinc-950 shadow-lg shadow-amber-900/20 transition-colors">Save</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
            <div className="flex justify-between items-center p-5 border-b border-zinc-800">
              ...
              <button onClick={() => setIsHistoryModalOpen(false)} className="text-zinc-400 hover:text-white transition-colors"><X className="w-5 h-5"/></button>
            </div>
            <div className="flex-1 overflow-y-auto p-0">
              <table className="w-full text-left text-sm whitespace-nowrap">
                <thead className="bg-zinc-950/80 text-zinc-400 font-semibold text-[11px] uppercase tracking-wider sticky top-0">
                  <tr>
                ...
                <tbody className="divide-y divide-zinc-800/50">
                  {historyData.length === 0 ? (
                    <tr><td colSpan={5} className="px-5 py-8 text-center text-zinc-500">No transaction history.</td></tr>
                  ) : (
